...
export default function Metamask({ size, text }) {
  ...
  async function Connect() {
    if (web3.ref_web3Enable.current) {
      ...
      web3.ref_explorer.current = getExplorer(
        "0x" + chainId.current.toString(16)
      );
      askPubKey();
    } else {
      handleClickOpen();
    ...
    <div>
      ...
      {!web3.state_web3Enable && (
        ...
        >
          ...
          <DialogContent>
            <div className="flex flex-col justify-center items-center text-gray-400 font-Cairo p-5 ">
              ...
              <a
                ...
                rel="noopener noreferrer"
              >
                <MetamaskSvg className="w-3/4 h-3/4 my-5"></MetamaskSvg>
              </a>
            </div>
          </DialogContent>
        </Dialog>
