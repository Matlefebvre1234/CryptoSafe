...
import { Button,Dialog,DialogTitle,DialogContent,IconButton, TextField, InputAdornment, Checkbox, FormControlLabel} from "@mui/material";
...
export default function CreateAccount({ callback,setHashDoubleSecurityy,setStateDoubleSecurity}) {
  ...
  async function createAccount() {
    ...
    callback().then(async (test)=>{
      ...
      let contractAccount = new Contract(passwordManager.ref_account.current,abiAccount,signer);
      let hash = ethers.utils.id(web3.ref_address.current+ inputPassword.current);
        ...
        <DialogContent className="flex flex-col items-center justify-center  p-5">
          ...
          {!loading && (
            <div className="flex flex-col">
              <p className="text-justify mt-10 font-Cairo leading-normal">
                Do you want to enable Double security feature ? Double security is a unique password that you need in addition to your wallet to gain access to the account. It help protect your data, if in a rare case someone steal
                your private key.The password will be use to encrypt your data a second time to make sure only you can access your passwords. This feature can be activate later, but a reset of the account will be necessary, so it's better now then later.
              </p>

              <FormControlLabel className={checkboxState ? "mt-2" : "my-8"} control={<Checkbox checked={checkboxState} onClick={()=>{setCheckBoxState(prev => !prev)}}></Checkbox>} label='Enable Double security'></FormControlLabel>
             {checkboxState && <div className="flex flex-col justify-center items-center"> <span className="font-Cairo text-sm">Don't lose this password because you want be able to gain acess to your account</span>  <TextField
                required
                size="small"
                error={errorPassword}
                onChange={(e) => {
          ...
          {!loading && (
            <Button
              size="medium"
              variant="contained"
              className="bg-blue-800 hover:bg-blue-400 m-2 w-3/4  font-Concert hover:transform hover:scale-105"
              onClick={validInput}
            >
              Create Account
            </Button>
